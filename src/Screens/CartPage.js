...
const GLOBAL = require('../../Global');
  ...
  export default class cartPage extends Component{
      ...
      async increment(id){fetch(GLOBAL.BASE_URL+"cartitems/increment/",{
          method:"POST",
          body:JSON.stringify({
              productId:id,
      ...
      async componentDidMount(){
          fetch(GLOBAL.BASE_URL+"cartitems?userid="+await AsyncStorage.getItem("userid"))
              .then(res => res.json())
              .then(
                  (result) => {this.setState({allProducts:result});
      ...
      //onclick to payment and checkout
                      ...
                      renderItem={({item})=>(
                          <View  style={{flexDirection:'row',height:120,marginTop:10,backgroundColor:'white', borderWidth:1,borderColor: '#ddd'}} >
                              ...
                              <View style={{paddingLeft:280,paddingTop:50,flexDirection:'row',position:'absolute'}}>
                                  <AntDesign name="plussquareo" size={15} onPress={()=>{this.increment(item.productId)}} >
                                      <Text>{item.Quantity}</Text>
                                  </AntDesign>
                                  <AntDesign name="minussquareo" size={15} onPress={()=>{((item.Quantity>1)?this.decrement(item.productId):this.removeItem(item.productId))}}>
                                  </AntDesign>
                              </View>
                          </View>
                      )}
